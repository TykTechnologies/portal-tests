...
import { PRODUCT_PUBLIC_NAME, PRODUCT_PRIVATE_NAME, CUSTOM_CATALOGUE_TEAM_B_NAME, PRODUCT_TEAM_B_NAME, GOLD_PLAN_NAME } from '../../../config_variables';

describe('Catalogues visiblity - creating new catalogues', () => {
    const expectedCountOfPublicProducts = 1;
    ...
    it('New catalogue is not visible for not-logged users', () => {
        admin_page.logOut();
        portal_page.CATALOGUES_BUTTON.click();
        expect(cataloguesCardsArray).to.have.lengthOf(expectedCountOfPublicProducts, "Incorrect number of visible catalogues!");
        const cataloguesNames = dev_catalogues_page.getNamesOfDisplayedProducts();
        expect(cataloguesNames).not.to.have.members([catalogueTeamBDetails.name]);
    });
    ...
    it('New catalogue should be visible for devB@tyk.io user', () => {
        ...
        portal_page.CATALOGUES_BUTTON.click();
        const cataloguesNames = dev_catalogues_page.getNamesOfDisplayedProducts();
        expect(cataloguesNames).to.have.members([PRODUCT_PUBLIC_NAME, PRODUCT_PRIVATE_NAME, catalogueTeamBDetails.name]);
    });
