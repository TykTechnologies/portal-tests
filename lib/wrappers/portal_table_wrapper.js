...
/**
 * Representing Portal table object
 * @class
 */
class Portal_table_object extends Table_object {
 ...
 * @param {int} rowNumber row number, 0-based counting
 ...
 getCellElementByColumnNameAndRowNumber(columnName, rowNumber) {
  ...
  expectCellWithTextToBeDisplayed(cellText) {
    return wdioExpect($(this.selector).$(`div*=${cellText}`)).toBeDisplayed();
  }

  expectCellWithTextNotToBeDisplayed(cellText) {
    return wdioExpect($(this.selector).$(`div*=${cellText}`)).not.toBeDisplayed();
  }
}

module.exports = Portal_table_object;