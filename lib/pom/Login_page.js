...
import { URL, LOGIN_PATH, USER_EMAIL, USER_PASSWORD, DEV_EMAIL, DEV_A1_EMAIL, DEV_A_EMAIL, DEV_PASS, DEV_B_EMAIL, DEV_D_EMAIL } from '../../config_variables';

class Login_page extends Page {
    
    get USERNAME_INPUT() {return $('input[name="login"]');}
    ...
    loginAsDevA1(){
      this.login(DEV_A1_EMAIL, DEV_PASS);
    }

    loginAsDevB(){
      this.login(DEV_B_EMAIL, DEV_PASS);
    }

    loginAsDevD(){
      this.login(DEV_D_EMAIL, DEV_PASS);
    }
